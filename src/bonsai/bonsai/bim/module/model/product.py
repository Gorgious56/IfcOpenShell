...
class CopyZRotationToSelected(bpy.types.Operator, tool.Ifc.Operator):
    """Apply active's Z rotation to other selected objects; Shift to flip 180°."""
    ...
    flip: bpy.props.BoolProperty(
        name="Flip 180°",
        description="Add 180° to the source Z rotation before applying",
        default=False,
    )
    ...
    def _execute(self, context):
        ...
        if not targets:
            return {"CANCELLED"}
        rotated = bonsai.core.product.copy_z_rotation_to_selected(
            tool.Ifc,
            tool.Geometry,
            tool.Surveyor,
            active=active,
        ...
        if rotated:
            self.report({"INFO"}, f"Aligned Z rotation of {rotated} object(s) to active{suffix}.")
        else:
...
class GizmoCopyZRotation(bpy.types.GizmoGroup, gizmo.BaseIconActionGroup):
    """Icon-action gizmo for CopyZRotationToSelected; shown when active is an IfcWall."""
    ...
    def is_eligible_object(cls, obj):
        # Active must be a wall.
        selected = list(tool.Blender.get_selected_objects())
        if len(selected) != 2:
            return False
        # Suppress when both selected are walls — wall-join group already
        # occupies the intersection.
        wall_pair = all((e := tool.Ifc.get_entity(o)) is not None and e.is_a("IfcWall") for o in selected)
        if wall_pair:
            return False
        element = tool.Ifc.get_entity(obj)
        return element is not None and element.is_a("IfcWall")

    def get_icon_anchor(self, context):
        active = context.active_object
        if active is None:
            return None
        target = next(
