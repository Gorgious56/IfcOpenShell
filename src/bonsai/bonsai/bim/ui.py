...
class GizmoPreferencesFeature(bpy.types.PropertyGroup):
    """Per-feature visibility toggle for a parametric gizmo group."""

    enabled: BoolProperty(
        name="Enabled",
        default=True,
        description="Show this feature's gizmos in the 3D viewport",
    )
...
class GizmoPreferences(bpy.types.PropertyGroup):
    """Aggregator for parametric gizmo visibility settings."""
    ...
    door: bpy.props.PointerProperty(type=GizmoPreferencesFeature)
    window: bpy.props.PointerProperty(type=GizmoPreferencesFeature)
    stair: bpy.props.PointerProperty(type=GizmoPreferencesFeature)
    wall: bpy.props.PointerProperty(type=GizmoPreferencesFeature)
    roof: bpy.props.PointerProperty(type=GizmoPreferencesFeature)
    railing: bpy.props.PointerProperty(type=GizmoPreferencesFeature)
    array: bpy.props.PointerProperty(type=GizmoPreferencesFeature)
    pipe_segment: bpy.props.PointerProperty(type=GizmoPreferencesFeature)
    duct_segment: bpy.props.PointerProperty(type=GizmoPreferencesFeature)

    if TYPE_CHECKING:
        draw_gizmos_in_3d_viewport: bool
        door: GizmoPreferencesFeature
        window: GizmoPreferencesFeature
        stair: GizmoPreferencesFeature
        wall: GizmoPreferencesFeature
        roof: GizmoPreferencesFeature
        railing: GizmoPreferencesFeature
        array: GizmoPreferencesFeature
        pipe_segment: GizmoPreferencesFeature
        duct_segment: GizmoPreferencesFeature
...
class BIM_ADDON_preferences(bpy.types.AddonPreferences):
    ...
    def draw_gizmo_parameters(self, layout: bpy.types.UILayout, context: bpy.types.Context) -> None:
        ...
        for feature in tool.Parametric.EDIT_TYPES:
            feature_prefs = getattr(self.gizmos, feature.name, None)
            if feature_prefs is None:
                continue
            box.prop(feature_prefs, "enabled", text=feature.name.replace("_", " ").title())
