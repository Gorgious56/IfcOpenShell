...
def test_extend_preview_line_renders_extension_when_cursor_past_end():
    ...
    the cursor's projected length. World math: identity matrix, so local-Z
    maps 1:1 to world-Z. Pin the endpoints exactly."""
    from bonsai.bim.module.model.decorator import MEPSegmentExtendPreviewDecorator
...
def test_extend_preview_line_renders_trim_when_cursor_inside_segment():
    ...
    projected (shorter) length. The line's direction matters less than the
    fact that it spans the trim distance — both endpoints must be correct."""
    from bonsai.bim.module.model.decorator import MEPSegmentExtendPreviewDecorator
...
def test_extend_preview_line_clamps_cursor_projection_to_minimum():
    ...
    result = MEPSegmentExtendPreviewDecorator._compute_extend_preview_line(
        matrix_world=Matrix.Identity(4),
        cursor_world=Vector((0.0, 0.0, -2.0)),  # cursor behind the segment
        current_length=1.0,
        min_projected_length=0.01,
    )
    assert result is not None
...
def test_extend_preview_line_respects_object_rotation():
    """A rotated segment (e.g. horizontal pipe with 90° rotation around Y)
    should produce world-space endpoints rotated accordingly. Pin so a
    future refactor doesn't drop the matrix_world multiplication."""
    import math

    from bonsai.bim.module.model.decorator import MEPSegmentExtendPreviewDecorator

    # 90° rotation around Y axis: local +Z maps to world +X.
    rotation = Matrix.Rotation(math.pi / 2, 4, "Y")
    result = MEPSegmentExtendPreviewDecorator._compute_extend_preview_line(
        matrix_world=rotation,
        cursor_world=Vector((3.0, 0.0, 0.0)),  # world +X at distance 3
        current_length=1.0,
        min_projected_length=0.01,
    )
    assert result is not None
